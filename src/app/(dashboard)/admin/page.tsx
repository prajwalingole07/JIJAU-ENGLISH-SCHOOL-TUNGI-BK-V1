    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100 font-semibold text-gray-500">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <Card 
              key={stat.title} 
              className="border-none shadow-sm h-[160px] flex items-center bg-white rounded-2xl group hover:shadow-md transition-all cursor-pointer active:scale-95"
              onClick={() => router.push(stat.path)}
            >
              <CardContent className="p-8 w-full flex items-center justify-between">
            ...
            <CardContent className="p-8 flex items-center justify-center min-h-[350px]">
              <div className="w-full h-64 bg-gray-50 rounded-2xl border border-dashed border-gray-200 flex items-center justify-center">
                 <div className="flex flex-col items-center gap-4 text-gray-400">
                    <div className="w-48 h-48 rounded-full border-[16px] border-[#EEF3F5] relative">
                ...
                {students.slice(-4).reverse().length === 0 ? (
                  ...
                  students.slice(-4).reverse().map((student) => (
                    <div key={student.id} className="flex items-center gap-4 group">
                      <div className="h-10 w-10 rounded-full bg-blue-50 flex items-center justify-center text-primary font-bold">
